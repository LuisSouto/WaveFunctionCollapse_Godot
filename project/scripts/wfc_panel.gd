...
func _ready():
	...
	image_container.pixel_pos_label.visible = view_button.get_popup().is_item_checked(ViewMenu.ViewButtons.PIXEL_COORDINATES)
...
func on_view_button_index_pressed(index: int) -> void:
	match index:
		...
		ViewMenu.ViewButtons.PIXEL_COORDINATES:
			image_container.pixel_pos_label.visible = view_button.get_popup().is_item_checked(ViewMenu.ViewButtons.PIXEL_COORDINATES)
